...
case "$ROL" in
  lb)
    ...
    ;;
  app)
    ufw allow from 10.10.0.0/16 to any port 8080  # Solo LAN puede acceder
    ;;
  db)
    ufw allow from 10.10.0.0/16 to any port 3306  # MySQL – solo LAN
    ufw allow from 10.10.0.0/16 to any port 6379  # Redis – solo LAN
    ;;
esac
